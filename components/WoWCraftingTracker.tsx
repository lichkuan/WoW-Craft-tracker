...
const WoWCraftingTracker: React.FC = () => {
  ...
  const categorizeItem = (name: string): string => {
    ...
    if (
      lower.includes("arme") ||
      lower.includes("épée") ||
      lower.includes("hache")
    )
      return "Armes";
    if (lower.includes("bottes") || lower.includes("chaussures"))
      return "Bottes";
                              ...
                              <div className="flex flex-wrap gap-1 mt-1">
                                {recipe.crafters.map((crafter) => (
                                  <button
                                    key={crafter}
                                    onClick={() => {
                                      const publicChar = publicCharacters.find(
                                        (c) => c.name === crafter,
                                      );
                                      if (publicChar) {
                                        window.open(
                                          `?share=${publicChar.shareId}`,
                                          "_blank",
                                        );
                                      }
                                    }}
...
- [Autre Item](https://wowhead.com/cata/spell=67890)"
  ...
  const CharacterView = () => {
    ...
    const professionsArray = [
      currentCharacter.profession1,
      currentCharacter.profession2,
    ].filter(Boolean);
    ...
    const filteredProfessionData = useMemo(() => {
      return professionsArray.map((profession) => {
        ...
        const categories = filteredCrafts.reduce(
          (acc, craft) => {
    ...
    return (
      <div className="max-w-6xl mx-auto">
        ...
        {filteredProfessionData.map(({ profession, crafts, categories }) => (
          <div
            key={profession}
            className="bg-gray-800 rounded-lg border border-yellow-600 mb-6"
          >
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  {(currentCharacter.professionLevels?.[profession] || 0) >
                    0 && (
                    <p
                      className={`text-sm ${getProfessionLevelColor(currentCharacter.professionLevels[profession])}`}
                    >
          ...
          <div className="space-y-4 text-gray-200">
            ...
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 ml-4">
                ...
                <li>
                  Tapez :{" "}
                  <code className="bg-gray-700 px-2 py-1 rounded text-yellow-300">
                    /tsexport markdown
                  </code>
                </li>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              {characters.map((character) => (
                <div
                  key={character.id}
                  className="bg-gray-700 rounded-lg p-4 border border-gray-600 hover:border-yellow-500"
                >
                  <div className="flex items-center justify-between">
                    <div
                    ...
                    {character.profession1 && (
                      <div className="flex items-center justify-between bg-gray-600 rounded p-2">
                        <div className="flex flex-col">
                          ...
                          {(character.professionLevels?.[
                            character.profession1
                          ] || 0) > 0 && (
                            <span
                              className={`text-xs ${getProfessionLevelColor(character.professionLevels[character.profession1])}`}
                            >
                              {getProfessionLevelIcon(
                                character.professionLevels[
                                  character.profession1
